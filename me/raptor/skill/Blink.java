				...
				if (p.isSneaking()) {
					...
					if (click == 3) {
						if (Cooldown.isOnCooldown(p, "Blink")) {
							p.sendMessage(ChatColor.RED + "You can use Blink after " + Cooldown.getTimeLeft(p, "Blink"));
							return;
						}
						Cooldown.setCooldown(p, "Blink", 8f);
						if (hunger <= 6) {
							p.sendMessage(ChatColor.YELLOW + "Hunger too low!");
							return;
						} else {
						click = 0;
						chooseTargetRightClick(p);
						new BukkitRunnable() {
							@Override
							public void run() {			
								blink(p);
